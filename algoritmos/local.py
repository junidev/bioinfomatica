...
fileVariable = open('./static/output/local.txt', 'r+')
fileVariable.truncate(0)
fileVariable.close()

def writeResult(seq1, seq2, match=2, missmatch=-2, gap=-2):
    alinear = AligmentLocal(seq1,seq2)
    vecA, vecB = alinear._local(match,missmatch,gap)
    vecA_string = ''.join(map(str, vecA))
    print(vecA_string)
    vecB_string = ''.join(map(str, vecB))
    print(vecB_string)

    '''with open("./static/output/local.txt", "w") as out:
        out.write(vecA_string + '\n')
        out.write(vecB_string + '\n')
    exit()'''

if ".fasta" in s1 and ".fasta" in s2:
    fasta1 = SeqIO.parse(s1, "fasta")
    fasta2 = SeqIO.parse(s2, "fasta")
    seq1 = ''
    seq2 = ''
    for i, j in zip(fasta1, fasta2):
        seq1 += i.seq.upper()
        seq2 += j.seq.upper()
    writeResult(seq1, seq2, int(s3), int(s4), int(s5))
else:
    writeResult(s1, s2, int(s3), int(s4), int(s5))
